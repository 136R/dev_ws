...
| **里程计几何的真相源是 `hw_controllers.yaml`，不是固件** | 去改固件里的 `WHEEL_RADIUS` / `WHEEL_TREAD` 再重烧 —— 白做，里程计一点不变 | 那两个宏全固件只被 `motor_controller.c` 的 `motor_controller_set_velocity_debug()` 引用，而该函数**无任何调用点**（grep 一下就知道）；`main.c` 走 `motor_controller_set_target()` 直接收轮角速度，`stm32_serial_hw.cpp` 下发/回读也不经轮径换算 |
| **机器人几何有三份副本，必须同步** | 轮距/轮径不一致 → 里程计系统性偏差，怎么调 SLAM 都白搭 | 三处的 `wheel_separation` / `wheel_radius` 必须相等：① `description/*.xacro` ② `my_bot_hw/config/hw_controllers.yaml`（实机真相源）③ `my_bot/config/my_controllers.yaml`（sim 侧） |
| **`base_link` 不在车体圆心上** | 拿 `robot_radius` 当外廓用 → 前方虚胖近 10 cm，窄门过不去 | 车体是圆盘，但 `base_link` 必须落在两轮轴中点，圆心在其**后方**（直径与偏移量见 `description/*.xacro`）。Nav2 五份配置一律用多边形 `footprint`，不用 `robot_radius` |

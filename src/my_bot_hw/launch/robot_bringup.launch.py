...
def generate_launch_description():
    ...
    robot_description_content = ParameterValue(
        Command([
            ...
            # 雷达 yaw 外参。2026-08-13 对墙复标(V9): 车两轮到墙等距摆正、0.8~1.2 m、
            # 拟合 /scan_filtered 前方 ±20° 墙面法向 —— yaw=pi 时偏 +1.352°(std 0.041°)，
            # 用此值时残余 -0.047°(std 0.032°)，远小于 0.30° 的摆正误差地板 => 保持不动。
            #
            # 注意: 底盘重做时曾判定"此值随整车重装作废"并复位为名义 pi，那个判断是错的。
            # 该偏差来自雷达零位/安装座的机械角度，与底盘几何无关，整车重装不改变它。
            # 残余 0.047° 已在噪声内，再按脚本输出精调就是在拟合噪声，别动。
            ' lidar_yaw:=3.115414361',
        ]),
        value_type=str
    )

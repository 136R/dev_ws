...
| 雷达波特率   | `460800`             | C1 专用                                    |
| 轮间距       | `0.192 m`            | `hw_controllers.yaml` 的 `wheel_separation` |
| 轮半径       | `0.035 m`            | `hw_controllers.yaml` 的 `wheel_radius`（轮径 0.07 m） |
...
| 最大线速度   | 不在此记数值         | `= WHEEL_MAX_RAD_S (27.2) × 有效轮半径`，有效半径 = `wheel_radius × radius_multiplier`，随标定变，只看 `hw_controllers.yaml` |

> 传动链常数（减速比 / 编码器 / 控制环）取自
> `firmware/my_robot_stm32/Core/Inc/app/robot_config.h`。
> **但轮距与轮半径的真相源是 `config/hw_controllers.yaml`，不是固件** —— 固件里的
> `WHEEL_TREAD` / `WHEEL_RADIUS` 只服务于一个无调用点的 debug 函数，改了对运行零影响，
> 见 `my_bot/docs/README.md` 不变量表。

...
>
> 🚨 **`src/sllidar_ros2/` 在 `.gitignore:38` 里，不受版本管理。**
> 后果：下面这些改动**只存在于各台机器的磁盘上**，`git push pi main` 带不过去，
> 换机器 / 重装系统 / 误删目录都会静默丢失，而且丢了之后**没有任何东西会报错**
> —— 症状是转向时 costmap 障碍又开始偏 15 cm，没人会想到是这里。
> 本文件是唯一的线索，所以它必须放在**被跟踪的** `docs/vendor/` 下而不是包目录里。
>
> 同步方式（唯一）：`scp src/sllidar_ros2/src/sllidar_node.cpp orangepi:~/dev_ws/src/sllidar_ros2/src/`
> 改完**两边都要重编**。
>
> **建议**：把这个包纳入版本管理（删掉那条 ignore 规则）。有了本地补丁之后，
> 「不跟踪」的代价已经从"少一个大目录"变成"补丁会无声丢失"。
